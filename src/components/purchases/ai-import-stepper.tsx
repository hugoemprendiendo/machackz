...
import { Loader2, UploadCloud, Wand2, PlusCircle, Trash2, ArrowRight, ArrowLeft, Save } from "lucide-react";
...
export function AiImportStepper() {
    ...
    const [itemToCreate, setItemToCreate] = React.useState<{ name: string, index: number } | null>(null);
        ...
        reader.onload = async () => {
            try {
                ...
                const newItems = result.items.map(item => {
                    const existingProductId = findClosestMatch(item.name, physicalInventory);
                    return {
                ...
                toast({ title: "Factura Analizada", description: "Revisa los productos encontrados y créalos si es necesario." });
                setStep(2);
            } catch (error) {
    ...
    return (
        <>
            ...
            {step === 1 && (
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Selecciona el proveedor y el archivo de la factura (XML o imagen) para comenzar.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid md:grid-cols-2 gap-6 items-start">
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <Input ref={fileInputRef} type="file" className="hidden" accept="image/*,application/xml,text/xml,.pdf" onChange={(e) => setInvoiceFile(e.target.files?.[0] || null)} />
                            </div>
                        </div>
                    </CardContent>
                    <CardFooter>
            ...
            {step === 2 && (
                <Card>
                    <CardHeader>
                        <CardTitle>Paso 2: Revisa los Productos</CardTitle>
                        <CardDescription>Verifica los productos encontrados por la IA. Si un producto es nuevo, créalo para poder continuar.</CardDescription>
                    </CardHeader>
                    <CardContent>
                         <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                                ...
                                <TableRow key={field.id}>
                                    <TableCell>
                                        {field.itemNameFromAI ? (
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm text-amber-600 flex-1">Nuevo: "{field.itemNameFromAI}"</span>
                                                 <Button type="button" size="sm" onClick={() => handleCreateProduct(index, field.itemNameFromAI!)}>
            ...
            {step === 3 && (
                 <Card>
                    ...
                    <CardFooter className="justify-between">
                         ...
                         <Button onClick={form.handleSubmit(onSubmit)}><Save className="mr-2 h-4 w-4"/>Confirmar y Registrar Compra</Button>
                    </CardFooter>
                 </Card>
            )}
